...
const signup = async (username, email, password, termsAccepted) => {
	...
	try {
		...
		await setDoc(doc(db, 'users', user.uid), {
			...
			lastSeen: Date.now(),
			deleteAt: null, //  Pole do obsługi opóźnionego usunięcia konta
		})
...
//  **Oznaczenie konta do usunięcia z 7-dniowym okresem karencji**
const markUserForDeletion = async () => {
	try {
		...
		deleteAt.setDate(deleteAt.getDate() + 7) //  7 dni karencji
...
//  **Sprawdzenie statusu konta (przywrócenie lub usunięcie po 7 dniach)**
		...
		if (userDoc.exists()) {
			...
			if (userData.deleteAt) {
				...
				if (deleteDate > new Date()) {
					//  Użytkownik zalogował się przed upływem 7 dni - anulujemy usunięcie
					await updateDoc(doc(db, 'users', userId), { deleteAt: null })
					toast.success('Usunięcie konta zostało anulowane.')
				} else {
					//  7 dni minęło - trwałe usunięcie konta
...
//  **Usunięcie danych użytkownika + anonimizacja wiadomości**
const deleteUserData = async userId => {
	try {
		await deleteDoc(doc(db, 'users', userId))

		//  Anonimizacja wiadomości
...
export {
	...
	logout,
	resetPass,
	markUserForDeletion, //  Dodana funkcja do usunięcia konta
